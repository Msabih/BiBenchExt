...
class BNNConv2d(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            x = torch.sign(x).to(torch.int8)
            #real_weights = self.weight.view(self.shape)
            #binary_weights = torch.sign(real_weights)
            y = F.conv2d(x, self.weight.to(torch.int8), stride=self.stride, padding=self.padding,bias=self.bias)
...
class BNNConv1d(nn.Module):
    ...
    def forward(self, x):
        if self.training:
           ...
           # real_weights = self.weight.view(self.shape)
            #binary_weights = torch.sign(real_weights)
            y = F.conv1d(x, self.weight.to(torch.int8), stride=self.stride, padding=self.padding)
            return y.to(torch.float32)
